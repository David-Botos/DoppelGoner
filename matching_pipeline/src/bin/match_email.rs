...
async fn main() -> Result<()> {
    ...
    info!("Starting email matching pipeline component");
    ...
    // Run the email matching component
    info!("Running email matching...");
    match matching::email::find_matches(&pool, Some(&reinforcement_orchestrator), &run_id).await {
        Ok(result) => {
            info!(
                "Email matching completed successfully. Created {} new entity groups.",
        ...
        Err(e) => {
            warn!("Email matching failed: {}", e);
            return Err(e);
        }
    }

    let elapsed = start_time.elapsed();
    info!("Email matching completed in {:.2?}", elapsed);
