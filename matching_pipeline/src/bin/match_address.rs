...
async fn main() -> Result<()> {
    ...
    info!("Starting address matching pipeline component");
    ...
    // Run the address matching component
    info!("Running address matching...");
    match matching::address::find_matches(&pool, Some(&reinforcement_orchestrator), &run_id).await {
        Ok(result) => {
            info!(
                "Address matching completed successfully. Created {} new entity groups.",
        ...
        Err(e) => {
            warn!("Address matching failed: {}", e);
            return Err(e);
        }
    }

    let elapsed = start_time.elapsed();
    info!("Address matching completed in {:.2?}", elapsed);
