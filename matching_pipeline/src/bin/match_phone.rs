...
async fn main() -> Result<()> {
    ...
    info!("Starting phone matching pipeline component");
    ...
    // Run the phone matching component
    info!("Running phone matching...");
    match matching::phone::find_matches(&pool, Some(&reinforcement_orchestrator), &run_id).await {
        Ok(result) => {
            info!(
                "Phone matching completed successfully. Created {} new entity groups.",
        ...
        Err(e) => {
            warn!("Phone matching failed: {}", e);
            return Err(e);
        }
    }

    let elapsed = start_time.elapsed();
    info!("Phone matching completed in {:.2?}", elapsed);
