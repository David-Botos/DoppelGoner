...
async fn main() -> Result<()> {
    ...
    info!("Starting name matching pipeline component");
    ...
    // Run the name matching component
    info!("Running name matching...");
    match matching::name::find_matches(&pool, Some(reinforcement_orchestrator), &run_id).await {
        Ok(result) => {
            info!(
                "Name matching completed successfully. Created {} new entity groups.",
        ...
        Err(e) => {
            warn!("Name matching failed: {}", e);
            return Err(e);
        }
    }

    let elapsed = start_time.elapsed();
    info!("Name matching completed in {:.2?}", elapsed);
