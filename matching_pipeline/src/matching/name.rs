...
pub async fn find_matches(pool: &PgPool) -> Result<NameMatchResult> {
    info!("Starting name-based entity matching...");
    ...
    info!("Found {} unprocessed entities for name matching", unprocessed_entities.len());
    ...
    let valid_entities: Vec<&Entity> = unprocessed_entities.iter()
    ...
    for entity in &valid_entities {
        ...
        let entity_embedding = org_embeddings.get(&entity.id).and_then(|opt_vec| opt_vec.as_ref());
    ...
    info!("Added {} entities to existing name groups", entities_added_to_existing);
    
    // 4. Process remaining entities to form new groups
    let remaining_entities: Vec<&Entity> = valid_entities.iter()
        .filter(|e| !processed_entities.contains(&e.id))
        .copied()
        .collect();
    
    info!("Creating new groups for {} remaining entities", remaining_entities.len());
    ...
    info!("Name matching completed in {:.2?}: {} total groups, {} entities matched", 
          start_time.elapsed(), total_groups, total_entities_matched);
...
async fn get_existing_name_groups(conn: &tokio_postgres::Client) -> Result<HashMap<String, Vec<EntityId>>> {
...
async fn create_name_groups(
    ...
    for i in 0..entity_count {
        for j in (i+1)..entity_count {
            ...
            // Calculate semantic similarity if embeddings available
            let semantic_score = match (org_embeddings.get(&entity_i.id), org_embeddings.get(&entity_j.id)) {
...
async fn add_entity_to_name_group(
    ...
    let result = async {
        ...
        if let Some(row) = method_row {
            ...
            conn.execute(update_method, &[&updated_values, &method_id]).await?;
        } else {
...
fn normalize_name(name: &str) -> String {
    ...
    let entity_types = [
        ...
        " lp", " llp", " foundation", " trust", " charitable trust", " co", " company",
        " non-profit", " nonprofit", " nfp", " association", " assn", " coop", " co-op",
        " cooperative", " npo", " organisation", " org", " organization", " coalition",
        " fund", " partnership", " academy", " consortium", " institute", " services",
        " group", " society", " network", " federation", " international", " global",
        " national", " alliance"
    ];
    ...
    normalized = abbr_regex.replace_all(&normalized, "association").to_string();
    
    let abbr_regex = Regex::new(r"\b(dept|dpt)\b").unwrap();
    normalized = abbr_regex.replace_all(&normalized, "department").to_string();
    
    let abbr_regex = Regex::new(r"\b(intl|int'l)\b").unwrap();
    normalized = abbr_regex.replace_all(&normalized, "international").to_string();
