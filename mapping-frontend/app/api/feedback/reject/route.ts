...
export async function POST(request: NextRequest) {
  try {
    ...
    // 1. Create human_review_decisions record with type REJECT_PAIRWISE_MATCH
    // 2. Create human_review_method_feedback record with was_correct=false
    // 3. (Not for MVP) Potentially trigger re-clustering
    
    console.log('Rejected link feedback:', {
      reviewer_id: feedback.reviewer_id,
      entity_group_id: feedback.entity_group_id,
      decision_type: DecisionType.REJECT_PAIRWISE_MATCH,
    ...
    console.error('Error processing reject feedback:', error);
    return NextResponse.json(
