...
use inference_worker::inference::model::{get_best_device, BgeEmbeddingModel, BgeTokenizer};
...
async fn test_load_tokenizer() {
    ...
    if let Ok(tokenizer) = tokenizer_result {
        ...
        let (input_ids, attention_mask) = tokenizer
            .tokenize_batch(&[test_text.to_string()], 512)
            .expect("Failed to tokenize text");

        // Check that we got non-empty tokens
        assert!(!input_ids.is_empty(), "Input IDs should not be empty");
        assert!(
            !input_ids[0].is_empty(),
            "Input IDs for the first text should not be empty"
        );
        assert!(
            !attention_mask.is_empty(),
            "Attention mask should not be empty"
        );
        assert!(
            !attention_mask[0].is_empty(),
            "Attention mask for the first text should not be empty"
        );

        // Verify attention mask is all 1s
        assert!(
            attention_mask[0].iter().all(|&x| x == 1),
...
async fn test_load_model() {
    ...
    if let Ok(model) = model_result {
        ...
        println!(
            "Successfully loaded model with {} embedding dimensions",
...
async fn test_tokenization_and_embedding() {
    ...
    if let Err(e) = model_result {
        ...
        if error_string.contains("no metal implementation for layer-norm") {
            println!(
                "ℹ️ This is a known limitation: Metal backend doesn't support layer normalization"
            );
    ...
    if let Err(e) = embeddings_result {
        ...
        if error_string.contains("no metal implementation") {
            ...
            println!(
                "ℹ️ Try running this test with the CPU backend: CANDLE_BACKEND=cpu cargo test"
            );
