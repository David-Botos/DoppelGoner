...
async fn test_engine_initialization() {
    ...
    assert_eq!(
        engine.is_busy(),
        false,
        "Engine should not be busy initially"
    );
    assert_eq!(
        engine.get_active_jobs(),
        0,
        "Engine should have 0 active jobs initially"
    );
    assert_eq!(
        engine.get_queue_depth(),
        0,
        "Engine should have 0 queue depth initially"
    );
    ...
    assert_eq!(
        ...
        "Engine should have the expected optimal batch size"
    );
    assert_eq!(
        capabilities.max_batch_size, 32,
        "Engine should have the expected max batch size"
    );
}
...
async fn test_circuit_breaker() {
    ...
    assert!(
        !engine.circuit_open,
        "Circuit should not be open after final reset"
    );
}
...
async fn test_engine_status_update() {
    ...
    assert!(
        engine.get_current_load() >= 0.0 && engine.get_current_load() <= 1.0,
        "Load should be between 0.0 and 1.0, got: {}",
...
async fn test_batch_tracking() {
    ...
    println!(
        "  Initial processing state for request {}: {}",
        request_id,
        engine.is_processing_batch(request_id)
    );
    assert!(
        !engine.is_processing_batch(request_id),
        "Should not be processing batch initially"
    );
    ...
    println!(
        "ℹ️ Note: A complete test would verify the engine is busy during actual batch processing"
    );
